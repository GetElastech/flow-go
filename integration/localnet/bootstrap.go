...
func prepareObserverService(i int, observerName string, agPublicKey string, profilerDir string, dataDir string) Service {
	...
	observerService := Service{
		Image: fmt.Sprintf("localnet-%s", DefaultAccessName),
		Command: []string{
			//fmt.Sprintf("--staked=false"), // This is here for the access image as observer
			fmt.Sprintf("--nodeid=%s", publicEndpointNodeID), // this can be removed for the real observer image
...
func prepareObserverServices(dockerServices Services, flowNodeContainerConfigs []testnet.ContainerConfig) Services {
	...
	for i := 0; i < observerCount; i++ {
		...
		dockerServices[observerName] = prepareObserverService(i, observerName, agPublicKey, profilerDir, dataDir)
