...
func (builder *ObserverNodeBuilder) Build() (cmd.Node, error) {
	builder.
		...
		Module("observer proxy", func(node *cmd.NodeConfig) error {
			...
			builder.rpcConf.UnsecureGRPCListenAddr = ":9000"
			node.Logger.Info().
				Str("observer_node", builder.rpcConf.UnsecureGRPCListenAddr).
				Msg(fmt.Sprintf("proxying %s to %s ...", builder.rpcConf.UnsecureGRPCListenAddr, "localnet_access_1_1:9000"))
			//proxy, err := NewFlowProxy("localnet_access_1_1:9000", builder.rpcConf.UnsecureGRPCListenAddr, builder.rpcConf.CollectionClientTimeout)

			proxy, err := NewFlowProxyStandalone("localnet_access_1_1:9000", nil, builder.rpcConf.CollectionClientTimeout)
			if err != nil {
				node.Logger.Error().
					Str("observer_node", err.Error()).
					Msg(fmt.Sprintf("proxying %s to %s failed", builder.rpcConf.UnsecureGRPCListenAddr, "localnet_access_1_1:9000"))
				return err
			}
			builder.Proxy = proxy
			return nil
